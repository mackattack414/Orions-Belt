...
**It’s important to provide the community-run Orion Controllers™ with some security advantage over regular one’s, as a way to sustain value for the membership and reward it.**

Again, these are high level ramblings, there’s no consideration, at this time, on how feasible they are, either technically or economically. But I hope you’ll agree that they represent relevant concerns and interesting features to have. We will discuss it within the community, for sure.
...
- Node owners will do the best they can to provide guaranteed bandwidth and latency in connections to other community-run nodes, thus allowing for a well-connected and consistent community-bound network – this is to be done without prejudice towards any other node, it will be a positive discrimination.

- Maybe there’s a way to setup a VPN-like network between community-run controllers, so that in the event of DDOS attacks or other disturbances, community nodes can still connect to each other in a proper way; this will have to be  a side connection, run along with regular public IP ones, maybe just a fallback method in case of need.

- community-run nodes may cooperate in setting up dedicated routes within the network, allowing for low latency paths that are not controlled by regular internet traffic governing, effectively creating a sort of Orion Controllers™ dedicated OSPF-like network path mesh that abstracts the underlying IP infrastructure.

- Public ports for the wallet’s and controller’s JSON/API services are unknown at this time, but it would be cool if the community could setup and run an obfuscation service that would allow tunneling the traffic over regular SSL to avoid snooping and ISP-based traffic shaping from taking over or blocking a node, like it happens with so many peer-to-peer connections.

- Having these advanced features available for regular, non-community-run nodes would also be important, so that the network as a whole is healthier, but it would require the ability to have at least a two tier structure on it –worth trying if we make it real and after having tier 1 working smoothly.
...
The Services part of the site, the one that’ll enable the participation and sharing of Orion Controllers™ and the other community-based efforts, it’s both tricky and amazing.
...
- Any service provided must use the aforementioned technologies on its core for it to be supported within the community and trough the website.
...
- The particular offer for biding the promotion of one of the community’s Orion Controllers™ to a Prime Controller™, using the community’s services, will require both the signing of enough funding in smart contracts and a qualified majority supporting vote (more than 75%) from the registered Orion Controllers™ owners within the community.

- Any trading between community members on these contracts and participations requires new smart contracts to both the deal itself and to the actual switching of property, on a back-to-back dual auto-escrowed contract of sorts.
...
Since the ideal platform for such transactions is not available to the community at the time – we’re talking about developing the thing here – and time is of the essence, a compromise is needed.
...
- A specific address will be created in a community-controlled wallet to support donations for the development of the Services framework and support.
...
- Preference will be given to community members that can claim and prove adequate development skills and experience in relevant projects, over outside professional contractors, on submissions for tender of the development.

- The community must decide and appoint a 3th-party auditor for code review before entering the effective usage of the system.

- The community will keep a running bounty on bug and exploit tracking of the system, paid for upon claim by member’s contributions – we’ll have to set up this proper, to avoid creating a monster, but it’s needed.
...
Either way, please go back to the  [thread on HT](https://hashtalk.org/topic/23234/ann-orion-belt-community-base-camp) and follow the link on the survey! or click [here](https://qtrial2011.qualtrics.com/SE/?SID=SV_81gxvH1x8VW0gsZ) I need your feedback on all of this!
